    ...
    class StdFilesystemService final : public ports::driven::IFilesystemService
    {
      public:
        ...
        void copy(const std::filesystem::path& sources,
                  std::filesystem::path& destination) const override;
        ...
        [[nodiscard]] bool exists(std::filesystem::path& target) const override;
        [[nodiscard]] bool isDirectory(std::filesystem::path& target) const override;
        [[nodiscard]] std::uintmax_t fileSize() const override;
    };
};  // namespace wendfyr::infrastructure
#endif
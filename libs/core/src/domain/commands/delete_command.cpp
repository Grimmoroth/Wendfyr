...
namespace wendfyr::domain::commands
{
    ...
    DeleteCommand::~DeleteCommand()
    {
        try
        {
            if (!_backup_directory.empty() && _fs.exist(_backup_directory))
            {
                ...
                spdlog::debug(
                    std::format("Cleaned up backup directory: {}",
                                _backup_directory.string()));
            }
        }
        catch (const std::exception& e)
        {
            spdlog::warn(
                std::format("Failed to clean up backup directory {}: {}",
                            _backup_directory.string(), e.what()));
        }
    }
    ...
    void DeleteCommand::execute()
    {
        ...
        _backup_directory =
            ...
            ("wf_backup_" + std::to_string(std::chrono::steady_clock::now()
                                               .time_since_epoch()
                                               .count()));
        ...
        for (const auto& target : _targets)
        {
            ...
            spdlog::debug(std::format("Deleting {} (backed up to {})",
                                      target.string(),
                                      backup_path.string()));
        ...
        spdlog::info(
            std::format("Delete complete: {} file(s)", _targets.size()));
    }

    void DeleteCommand::undo()
    {
        for (auto it{_backup_records.rbegin()};
             it != _backup_records.rend(); ++it)
        {
            const auto& [original, backup] = *it;
            if (_fs.exist(backup))
            {
                spdlog::debug(
                    std::format("Undo delete: restoring {} -> {}",
                                backup.string(), original.string()));
                _fs.move(backup, original);
            }
            else
            {
                spdlog::warn(std::format(
                    "Undo delete: backup {} missing, cannot restore",
                    backup.string()));
            }
        }

        std::vector<std::filesystem::path> restored;
        restored.reserve(_backup_records.size());
        for (const auto& [original, backup] : _backup_records)
        {
            restored.push_back(original);
        }

        _event_bus.publish(events::FilesCopiedEvent{
            .sources = restored,
            .destination = _targets.front().parent_path()});
        spdlog::info(
            std::format("Undo delete complete: {} file(s) restored",
                        _backup_records.size()));
    }

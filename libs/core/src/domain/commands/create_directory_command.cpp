...
namespace wendfyr::domain::commands
{
    ...
    void CreateDirectoryCommand::execute()
    {
        ...
        {
            spdlog::info(std::format("Directory already exist: {}",
                                     _dir_path.string()));
            _created = false;
            return;
        }
        ...
        _event_bus.publish(events::FilesCopiedEvent{
            .sources{_dir_path}, .destination{_dir_path.parent_path()}});

        spdlog::info(
            std::format("Created directory {} ", _dir_path.string()));
    }
    ...
    void CreateDirectoryCommand::undo()
    {
        ...
        {
            spdlog::info(
                "Undo create directory: skipping (may not created by "
                "Wendfyr)");
            return;
        }
        ...
        {
            spdlog::warn(
                std::format("Undo create directory: {} already gone",
                            _dir_path.string()));
            _created = false;
            return;
        }
        ...
        if (!_fs.listDirectory(_dir_path).empty())
        {
            spdlog::warn(std::format(
                "Undo create directory: {} is not empty, skipping",
                _dir_path.string()));
            return;
        }
        ...
        _event_bus.publish(
            events::FilesDeletedEvent{.deleted_paths{_dir_path}});

        spdlog::info(std::format("Undo create directory: removed {}",
                                 _dir_path.string()));
    }

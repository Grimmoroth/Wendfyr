...
namespace wendfyr::domain
{
    ...
    {
      public:
        PanelModel(ports::driven::IFilesystemService& fs,
        ...
        [[nodiscard]] std::filesystem::path currentDirectory()
            const override;
        [[nodiscard]] std::span<const domain::models::FileEntry> entries()
            const override;
        [[nodiscard]] std::size_t entryCount() const noexcept override;

        // Navigation
        void navigateTo(
            const std::filesystem::path& new_directory) override;
        void navigateUp() override;
        void refresh() override;

        // Sorting

        void sortBy(ports::driving::SortField field,
                    ports::driving::SortOrder order) override;
        [[nodiscard]] ports::driving::SortField currentSortField()
            const noexcept override;
        [[nodiscard]] ports::driving::SortOrder currentSortOrder()
            const noexcept override;
        ...
        selectedEntries() const override;
      ...
      private:
        ...
        ports::driving::SortField _sort_field{
            ports::driving::SortField::NAME};
        ports::driving::SortOrder _sort_order{

...
namespace wendfyr::domain
{
    ...
    {
      public:
        CommandFactory(ports::driven::IFilesystemService& fs,
                       services::EventBus& event_bus);

        [[nodiscard]] std::unique_ptr<domain::commands::ICommand>
        createCopyCommand(std::vector<std::filesystem::path> sources,
                          std::filesystem::path destination) override;

        [[nodiscard]] std::unique_ptr<domain::commands::ICommand>
        createMoveCommand(std::vector<std::filesystem::path> sources,
                          std::filesystem::path destination) override;

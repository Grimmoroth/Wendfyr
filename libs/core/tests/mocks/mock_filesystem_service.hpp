...
namespace wendfyr::tests::mocks
{
    ...
    class MockFilesystemService : public ports::driven::IFilesystemService
    {
      public:
        ...
        MOCK_METHOD(void, remove, (const std::filesystem::path& target),
                    (const, override));
        MOCK_METHOD(void, createDirectory,
                    (const std::filesystem::path& target),
                    (const, override));
        MOCK_METHOD(bool, exist, (const std::filesystem::path& target),
                    (const, override));
        MOCK_METHOD(bool, isDirectory,
                    (const std::filesystem::path& target),
                    (const, override));
        MOCK_METHOD(std::uintmax_t, fileSize,
                    (const std::filesystem::path& target),
                    (const, override));
    };

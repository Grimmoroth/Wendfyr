...
namespace wendfyr::tests
{
    ...
    class CopyCommandTest : public ::testing::Test
    {
      protected:
        ...
        {
            return std::make_unique<domain::commands::CopyCommand>(
                std::vector<std::filesystem::path>{source_file}, destination_dir, mock_fs,
                event_bus);
        }
    ...
    {
        EXPECT_CALL(mock_fs, copy(testing::_, testing::_)).Times(1);
            ...
            [&](const domain::events::Event& event)
            {
                std::visit(
                    domain::events::Overloaded{
                        ...
                        {
                            event_recived = true;
                            recived_destination = e.destination;
                        },
                        [](const auto&) {},
                    },
                    event);
    ...
    {
        auto expected_dir = destination_dir / source_file.filename();
    ...
    TEST_F(CopyCommandTest, RedoReExecutes)
    {
        ...
        EXPECT_CALL(mock_fs, copy(source_file, expected_dest)).Times(2);
        EXPECT_CALL(mock_fs, exist(expected_dest)).WillOnce(testing::Return(true));
        EXPECT_CALL(mock_fs, remove(expected_dest)).Times(1);
